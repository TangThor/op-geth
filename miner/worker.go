...
type generateParams struct {
	...
	parentHeight uint64            // Parent block height, empty means the latest chain head
...
func (w *worker) prepareWork(genParams *generateParams) (*environment, error) {
	...
	if genParams.parentHash != (common.Hash{}) {
		...
		parent = block.Header()
	} else if genParams.parentHeight != 0 {
		block := w.chain.GetBlockByNumber(genParams.parentHeight)
		if block == nil {
			return nil, fmt.Errorf("missing parent")
		}
		parent = block.Header()
...
func (w *worker) prepareWorkWithParent(genParams *generateParams, parent *types.Header, oldEnv *environment) (*environment, error) {
	w.mu.RLock()
	defer w.mu.RUnlock()
...
func (w *worker) simulateTx(txs []*types.Transaction, height uint64) ([]*core.ExecutionResult, error) {
	start := time.Now()
	...
	gp := core.GasPool(params.MaxGasLimit)
	env.gasPool = &gp

	snap := env.copy()
	defer func() {
		w.updateSnapshot(snap)
	}()
	results := []*core.ExecutionResult{}

	for _, tx := range txs {
		msg, err := core.TransactionToMessage(tx, types.MakeSigner(w.chainConfig, env.header.Number, env.header.Time), env.header.BaseFee)
		if err != nil {
			return nil, err
		}
		// Create a new context to be used in the EVM environment
		blockContext := core.NewEVMBlockContext(env.header, w.chain, &env.coinbase, w.chainConfig, env.state)
		txContext := core.NewEVMTxContext(msg)
		vmenv := vm.NewEVM(blockContext, txContext, env.state, w.chainConfig, *w.chain.GetVMConfig())
		result, err := core.ApplyMessage(vmenv, msg, env.gasPool)
		if err != nil {
			return nil, err
		}
		results = append(results, result)

		log.Info("commit tx success", "tx", tx, "result", result)

		//envSnap := env.copy()
		// Withdrawals are set to nil here, because this is only called in PoW.
		block, err := w.engine.FinalizeAndAssemble(w.chain, env.header, env.state, env.txs, nil, env.receipts, nil)
		if err != nil {
			log.Error("commit block error")
			break
		}
		...
		//w.pendingTasks[hash] = &task{receipts: env.receipts, state: env.state, block: block, createdAt: time.Now()}

		stopCh := make(chan struct{})
		err = w.engine.Seal(w.chain, block, w.resultCh, stopCh)
		if err != nil {
		...
		env, err = w.prepareWorkWithParent(&generateParams{
			timestamp: uint64(start.Unix()),
			coinbase:  coinbase,
		}, block.Header(), env)
		if err != nil {
